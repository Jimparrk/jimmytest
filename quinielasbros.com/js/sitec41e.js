...
function autocomplete(inputId, dataArray, onSelectItem) {
  ...
  input.addEventListener('input', function (e) {
    ...
    dataArray
      .filter(team => (team.name.substr(0, value.length).toUpperCase() == value.toUpperCase()))
      .forEach(team => {
        const itemContent = document.createElement('div');
        itemContent.innerHTML = '<img src="/flags/' + team.flag + '" height=25 class="mx-1" />';
        itemContent.innerHTML += '<strong>' + team.name.substr(0, value.length) + '</strong>';
...
function drawTable(tableId, data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    row.innerHTML = '<td class="text-center" width="45">'
      + '<input type="hidden" name="Matches[' + i + '].localTeamFlag" value="' + data[i].localTeamFlag + '">'
      + '<img src="/flags/' + data[i].localTeamFlag + '" class="mx-1 r-image" /></td>';
    row.innerHTML += '<td class="text-center">' + data[i].localTeamName
      + '<input type="hidden" name="Matches[' + i + '].LocalTeamName" value="' + data[i].localTeamName + '"></td>';
    row.innerHTML += '<td class="text-center">VS</td>';
    row.innerHTML += '<td class="text-center">' + data[i].visitorTeamName
      + '<input type="hidden" name="Matches[' + i + '].VisitorTeamName" value="' + data[i].visitorTeamName + '"></td>';
    row.innerHTML += '<td class="text-center" width="45">'
      + '<input type="hidden" name="Matches[' + i + '].VisitorTeamFlag" value="' + data[i].visitorTeamFlag + '">'
      + '<img src="/flags/' + data[i].visitorTeamFlag + '" class="mx-1 r-image" /></td>';
